...
@Data
@Service
public class GameSessionService extends AbstractEntityService<GameSession, GameSessionDocument> implements IGameSessionService {
    ...
    public GameSession createGameSession(GameSession gameSession) {
        ...
        GameSession newSession = new GameSession(gameSession.getTenant(), gameSession.getBrand(), gameSession.getPlayer(), gameSession.getCurrency());
        ...
        if (newSession.getJurisdiction() != null) {
            ...
            if (brand.getRealityCheckIntervalInMilliSeconds() > 0 && newSession.getRealityCheckIntervalInMilliSeconds() == 0)
    ...
    public GameSession findOneByGameConnectorAndCorrelationIdAndStatus(String gameConnector, String correlationId, Status status) {
        return getMapper().asDto(store.findOneByGameConnectorAndCorrelationIdAndStatus(gameConnector, correlationId, status));
    }
    ...
    public Boolean setExpiration(GameSession session) {
        ...
        //long elapsed = currentTimeInMilliseconds - session.getElapsedTimeInMilliSeconds();
        //log.info("currentTime {} elapsed time  {} ", currentTimeInMilliseconds, elapsed);
        if (session.getElapsedTimeInMilliSeconds() == 0
            //TODO fix this to reset elapsed time on every nth interval        || elapsed > session.getRealityCheckIntervalInMilliSeconds()
        ) {
            //  log.info("reset the elapsed time {} to {}. showing reality check popup", Duration.ofMillis(elapsed).toSeconds(), currentTimeInMilliseconds);
            session.setElapsedTimeInMilliSeconds(currentTimeInMilliseconds);
        }


        getRedisTemplate()
                .opsForValue().setGet(sessionKey, session.getElapsedTimeInMilliSeconds(), Duration.ofSeconds(getGameSessionExpirationSecs()));
        return true;
    }
    ...
    public GameSession createGameSession(GameLaunchRequest glr,
        ...
        if (glr.getLobbyUrl() != null) newSession.getUrls().put("lobbyUrl", glr.getLobbyUrl());
        if (glr.getDepositUrl() != null) newSession.getUrls().put("depositUrl", glr.getDepositUrl());
        if (glr.getHistoryUrl() != null) newSession.getUrls().put("historyUrl", glr.getHistoryUrl());

        return createGameSession(newSession);
    }

    public GameSession createGameSession(GameSession gameSession, String gameId, String player, String currency, String token) {
    ...
    public GameSession findOneByUidAcrossTenants(String uid) {
        return getMapper().asDto(getStore().getTemplate().findOne(Query.query(Criteria.where("uid").is(uid).and("deleted").is(false)), GameSessionDocument.class));
    }
