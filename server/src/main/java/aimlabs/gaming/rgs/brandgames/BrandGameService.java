...
public class BrandGameService extends AbstractEntityService<BrandGame, BrandGameDocument> implements IBrandGameService {
        ...
        public List<BrandGame> findAllByBrand(String brand) {
                return store.findAllByBrand(brand);
        }
        ...
        public BrandGameAggregate findOneByNetworkAndBrandAndGameId(String network, String brand, String gameId) {
                                                                ...
                                                                .append("pipeline", List.of(new Document("$match",
                                                                                new Document("$expr",
                                                                                                new Document("$and",
                                                                                                                Arrays.asList(
                                                                                                                                new Document("$eq",
                                                                                                                                                Arrays.asList("$$brand_id",
                                                                                                                                                                "$uid")),
                                                                                                                                new Document("$eq",
                                                                                                                                                Arrays.asList("$$tenant_id",
                                                                                                                                                                "$tenant")),
                                                                                                                                new Document("$eq",
                ...
                List<AggregationOperation> queryGameProvidersAggregation = aggregationPipeline
                                .map(document -> (AggregationOperation) context -> document)
                                .collect(Collectors.toList());

                AggregationResults<BrandGameAggregate> result = store.getTemplate().aggregate(
                                Aggregation.newAggregation(queryGameProvidersAggregation), "BrandGames",
                                BrandGameAggregate.class);
