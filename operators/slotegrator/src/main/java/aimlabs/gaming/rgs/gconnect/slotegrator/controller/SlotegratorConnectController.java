package aimlabs.gaming.rgs.gconnect.slotegrator.controller;
...
public class SlotegratorConnectController {
    ...
    @PostMapping(value = { "/game-url" })
        public ResponseEntity<GetGameUrlResponse> launch(
        ...
        try {
            ...
            if (remoteHost != null && !remoteHost.isBlank()) {
                if (gameUrlPath.startsWith("/")) {
                    gameUrlPath = "https://" + remoteHost + gameUrlPath;
            ...
            return ResponseEntity.status(HttpStatus.OK).body(new GetGameUrlResponse(location.toString()));
        } catch (Exception e) {
            log.error("Slotegrator Game launch failed!", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new GetGameUrlResponse(null));
        }
