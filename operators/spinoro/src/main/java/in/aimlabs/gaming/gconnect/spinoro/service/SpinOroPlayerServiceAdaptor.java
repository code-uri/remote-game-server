package in.aimlabs.gaming.gconnect.spinoro.service;
...
public class SpinOroPlayerServiceAdaptor
    ...
    class SprinOroPlayerServiceConnector extends AbstractConnectorWebClientBuilderService
        ...
        public Mono<PlayerInitialiseResponse> playerInitialise(PlayerInitialiseRequest request) {
            ...
            InitGameRequest initGameRequest = new InitGameRequest(
                    (String) getConnector().getSettings().getOrDefault("secret", "StagM00nSec122"),
                    request.getInternalToken(), request.getSessionToken(), request.getPlayer(), 1,
                    Integer.parseInt((String) getConnector().getSettings().getOrDefault("gameProviderId", "126")),
                    request.getGameId());
            ...
            return getWebClient()
                    ...
                    .tap(() -> new TapOnNextSignalListener<String>() {
                        public void doOnNext(String json) throws Throwable {
                            log.info("SpinOro Balance response json. {}", json);
                        }
                    })
                    .map(s -> {
                        try {
                            return objectMapper.readValue(s, InitGameResponse.class);
                        } catch (JsonProcessingException e) {
                            log.info("SpinOro Balance response json parse failed. {}", s, e);
                        }
                    ...
                    .tap(() -> new DefaultSignalListener<InitGameResponse>() {
                        public void doOnNext(InitGameResponse initGameResponse) throws Throwable {
                            log.info("SpinOro InitGame response. {}", initGameResponse);
                        }
                        ...
                        public void doOnError(Throwable error) throws Throwable {
                            log.error("SpinOro playerInfo request failed.", error);
                        }
                    })
                    .map(balanceRes -> {
                        ...
                        try {
                            ...
                            BigDecimal balanceAmt = BigDecimal.valueOf(balanceRes.balance)
            ...
            return getWebClient()
                    ...
                    .map(balanceRes -> {
                        ...
                        try {
                            ...
                            res.setBonus(new Balance().amount(BigDecimal.ZERO).onHold(BigDecimal.ZERO)
                                    .total(BigDecimal.ZERO));
                            res.setExternalToken(balanceReq.sessionId);
        ...
        public Mono<PlayerTransactionResponse> playerTransaction(PlayerTransactionRequest request) {
            ...
            List<Mono<Tuple2<TransactionResponse, String>>> txsFlux = new ArrayList<>();
            if (type == TransactionType.ROLLBACK) {
                txsFlux.add(processTxn(request.getToken(), request.getInternalToken(),
                ...
                if (type == TransactionType.DEBIT && request.getDebit() != null) {
                    Mono<Tuple2<TransactionResponse, String>> debitMono = processTxn(request.getToken(),
            ...
            return Flux.concat(txsFlux)
                    ...
                    .map(tuple2s -> {
                        ...
                        tuple2s.forEach(tuple2 -> {
                            ...
                            TransactionResponse res = tuple2.getT1();
                            String orgTxId = tuple2.getT2();

                            if (res.success) {
                                wallet.setTotalBalance(BigDecimal.valueOf(res.balance).divide(BigDecimal.valueOf(100),
        ...
        public Mono<PlayerTransactionResponse> rollback(PlayerTransactionRequest request) {
            ...
            return processTxn(request.getToken(), request.getInternalToken(),
                    ...
                    .map(tuple2 -> {
                        ...
                        if (res.success) {
                            ...
                            wallet.setTotalBalance(BigDecimal.valueOf(res.balance).divide(BigDecimal.valueOf(100), 2,
        ...
        private Mono<Tuple2<TransactionResponse, String>> processTxn(String sessionToken,
            ...
            if (type == TransactionType.DEBIT_CREDIT) {
                transactionRequest = new DebitCreditRequest((String) getConnector().getSettings()
                        .getOrDefault("secret", "StagM00nSec122"), internalToken, sessionToken, player, 1,
                        Integer.parseInt((String) getConnector().getSettings().getOrDefault("gameProviderId", "126")),
                        gameId, gameRoundId, List.of(
                                new Transaction("DEBIT", txnId + "_-1", null, cu.getCurrencyCode(),
                                        Double.valueOf(debit * 100D).longValue()),
                                new Transaction("CREDIT", txnId + "_1", null, cu.getCurrencyCode(),
                                        Double.valueOf(credit * 100D).longValue())),
                        roundClosed, null);
            } else {
                transactionRequest = new TransactionRequest((String) getConnector().getSettings()
                        .getOrDefault("secret", "StagM00nSec122"), internalToken, sessionToken, player, 1,
                        Integer.parseInt((String) getConnector().getSettings().getOrDefault("gameProviderId", "126")),
                        gameId, gameRoundId, null, txnId, cu.getCurrencyCode(),
            ...
            return getWebClient()
                    ...
                    // .publishOn(Schedulers.parallel())
                    .tap(() -> new DefaultSignalListener<TransactionResponse>() {
                        @Override
                        public void doOnNext(TransactionResponse response) {
                            log.info("{}", response);
                        }
                    })
                    ...
                    .flatMap(betResponse -> {
                        if (type == TransactionType.DEBIT && !betResponse.success) {
                    ...
                    .doOnError(throwable -> {
                        log.error("SpinOro transaction {} failed.", txnId, throwable);
                    })
