...
@RestController
@RequestMapping("/connect/v2")
@Data
@Slf4j
public class PlayerConnectV1Controller {
    ...
    public Mono<ResponseEntity<PlayerInitialiseResponse>> playerInitialise(
            @RequestBody Mono<PlayerInitialiseRequest> requestMono, ServerWebExchange exchange,
    ...
    public Mono<ResponseEntity<PlayerTransactionResponse>> playerTransaction(
            @RequestBody Mono<PlayerTransactionRequestV1> playerTransactionRequest, ServerWebExchange exchange,
            @RequestHeader(defaultValue = "default") String tenant) {

        return playerTransactionRequest
                .tap(() -> new DefaultSignalListener<>() {
                    @Override
                    public void doOnNext(PlayerTransactionRequestV1 request) {
                ...
                .flatMap(request -> {
                    ...
                    PlayerTransactionRequest req = new PlayerTransactionRequest();
                    req.setBrand(request.getBrand());
                    if (request.getCredit() != null) {
                        req.setCredit(request.getCredit().getAmount().doubleValue());
                        req.setTxnId(request.getCredit().getTxnId());
                    }

                    if (request.getDebit() != null) {
                        req.setDebit(request.getDebit().getAmount().doubleValue());
                        req.setTxnId(request.getDebit().getTxnId());
                    }
                    req.setCurrency(request.getCurrency());
                    req.setGameId(request.getGameId());
                    req.setGameRoundId(request.getGameRoundId());
                    req.setPlayerId(request.getPlayerId());
                    req.setGameRoundStatus(request.getGameRoundStatus());
                    req.setRequestType(request.getRequestType());
                    req.setTenant(request.getTenant());
                    req.setToken(request.getToken());
                    req.setFreeSpins(request.getFreeSpins());
                    req.setJackpotWinnings(request.getJackpotWinnings());
                    req.setOrgTxnUid(request.getOrgTxnUid());

                    return aggregatorPlayerServiceManager.playerTransaction(req);
                })
                .tap(() -> new DefaultSignalListener<>() {
                    @Override
